...
{
    [ITEM_ULTRA_BALL - ITEM_ULTRA_BALL]  = 25,
    [ITEM_GREAT_BALL - ITEM_ULTRA_BALL]  = 15,
    [ITEM_POKE_BALL - ITEM_ULTRA_BALL]   = 10,
    [ITEM_SAFARI_BALL - ITEM_ULTRA_BALL] = 35
};
...
void SetMoveEffect(bool8 primary, u8 certain)
{
    ...
    if (gBattleCommunication[MOVE_EFFECT_BYTE] <= PRIMARY_STATUS_MOVE_EFFECT)
    {
        ...
        if (statusChanged == TRUE)
        {
            ...
            {
                if (gBattleMons[gEffectBattler].ability == ABILITY_EARLY_BIRD)
                    gBattleMons[gEffectBattler].status1 |= STATUS1_SLEEP_TURN(3); // Always 1 unable-to-move turn for Early Bird
                else
                    gBattleMons[gEffectBattler].status1 |= STATUS1_SLEEP_TURN((Random() % 3) + 2); // 1-3 turns
            }
            else
...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_ON_DAMAGE_ABILITIES: // Such as abilities activating on contact (Effect Spore, Rough Skin, etc.).
            ...
            gBattleScripting.moveendState++;
            break;
        case MOVEEND_VENOMOUS:
            if (gBattleMons[gBattlerAttacker].ability == ABILITY_VENOMOUS
             && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             && gBattleMons[gBattlerAttacker].hp != 0
        ...
        case MOVEEND_SYNCHRONIZE_ATTACKER: // attacker synchronize
            ...
            gBattleScripting.moveendState++;
            break;
        case MOVEEND_CHOICE_MOVE: // update choice band / scarf / specs move
...
static void Cmd_pickup(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if (ability == ABILITY_PICKUP && species != SPECIES_NONE && species != SPECIES_EGG && heldItem == ITEM_NONE && !(Random() % 8))
        {

...
s32 CalculateBaseDamage(struct BattlePokemon *attacker, struct BattlePokemon *defender, u32 move, u16 sideStatus, u16 powerOverride, u8 typeOverride, u8 battlerIdAtk, u8 battlerIdDef)
{
    ...
    if (type == TYPE_FIRE && AbilityBattleEffects(ABILITYEFFECT_FIELD_SPORT, 0, 0, ABILITYEFFECT_WATER_SPORT, 0))
        gBattleMovePower /= 2;
    if (type == TYPE_GRASS && attacker->ability == ABILITY_OVERGROW && attacker->hp <= (attacker->maxHP / 5))
        gBattleMovePower = (150 * gBattleMovePower) / 100;
    if (type == TYPE_FIRE && attacker->ability == ABILITY_BLAZE && attacker->hp <= (attacker->maxHP / 5))
        gBattleMovePower = (150 * gBattleMovePower) / 100;
    if (type == TYPE_WATER && attacker->ability == ABILITY_TORRENT && attacker->hp <= (attacker->maxHP / 5))
        gBattleMovePower = (150 * gBattleMovePower) / 100;
    if (type == TYPE_BUG && attacker->ability == ABILITY_SWARM && attacker->hp <= (attacker->maxHP / 5))
        gBattleMovePower = (150 * gBattleMovePower) / 100;
            ...
            while (val != 0)
            {
                if (val & 1)
                {
                    ...
                    case 1: // ITEM4_EV_ATK
                        ...
                        {
                            // Lower EVs: allow even if at total EV cap; clamp at 0
                            if (data > 0)
                            {
                                s32 newData = (s32)data + (s8)itemEffect[idx];
                                if (newData < 0)
                                    newData = 0;
                                data = (u32)newData;
                        ...
                        {
                            // Raise EVs: respect per-stat and total EV caps
            ...
            while (curEffect)
            {
                if (curEffect & 1)
                {
                    ...
                    case 1: // ITEM4_EV_ATK
                        ...
                        if ((s8)itemEffect[idx] < 0)
                        {
                            // Lowering EVs has effect if current EV > 0
                            if (data > 0)
                                retVal = FALSE;
                            idx++;
                        }
                        ...
                        {
                            // Raising EVs: only if under caps

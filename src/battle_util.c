...
u8 TrySetCantSelectMoveBattleScript(void)
{
    ...
    if ((holdEffect == HOLD_EFFECT_CHOICE_BAND || holdEffect == HOLD_EFFECT_CHOICE_SCARF || holdEffect == HOLD_EFFECT_CHOICE_SPECS) && *choicedMove != MOVE_NONE && *choicedMove != MOVE_UNAVAILABLE && *choicedMove != move)
    {
...
u8 CheckMoveLimitations(u8 battlerId, u8 unusableMoves, u8 check)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        if ((holdEffect == HOLD_EFFECT_CHOICE_BAND || holdEffect == HOLD_EFFECT_CHOICE_SCARF || holdEffect == HOLD_EFFECT_CHOICE_SPECS) && *choicedMove != MOVE_NONE && *choicedMove != MOVE_UNAVAILABLE && *choicedMove != gBattleMons[battlerId].moves[i])
            unusableMoves |= gBitTable[i];
    }
            ...
            if (gBattleMons[battler].hp != 0)
            {
                ...
                case ABILITY_SOLAR_POWER:
                    if (WEATHER_HAS_EFFECT && (gBattleWeather & B_WEATHER_SUN))
                    {
                        ...
                        BattleScriptPushCursorAndCallback(BattleScript_SolarPowerDamage);
        ...
        for (battlerId = 0; battlerId < gBattlersCount; battlerId++)
        {
            ...
            case HOLD_EFFECT_CONFUSE_SOUR:
                    ...
                    && gBattleMons[battlerId].hp <= gBattleMons[battlerId].maxHP / 4)
                {
                    ...
                    if (battlerHoldEffect == HOLD_EFFECT_CONFUSE_SPICY)
                        flavor = FLAVOR_SPICY;
                    else if (battlerHoldEffect == HOLD_EFFECT_CONFUSE_DRY)
                        flavor = FLAVOR_DRY;
                    else if (battlerHoldEffect == HOLD_EFFECT_CONFUSE_SWEET)
                        flavor = FLAVOR_SWEET;
                    else if (battlerHoldEffect == HOLD_EFFECT_CONFUSE_BITTER)
            ...
            case HOLD_EFFECT_SP_DEFENSE_UP:
                    ...
                    && gBattleMons[battlerId].hp <= gBattleMons[battlerId].maxHP / battlerHoldEffectParam)
                {
                    ...
                    if (battlerHoldEffect == HOLD_EFFECT_DEFENSE_UP)
                        statId = STAT_DEF;
                    else if (battlerHoldEffect == HOLD_EFFECT_SPEED_UP)
                        statId = STAT_SPEED;
                    else if (battlerHoldEffect == HOLD_EFFECT_SP_ATTACK_UP)
                        statId = STAT_SPATK;
                    else
                        statId = STAT_SPDEF;
